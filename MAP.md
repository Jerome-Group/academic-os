...
| Curation register identity | The previewed cohort pass that brings a register's pre-v4 lines onto unnumbered-path plus sha-256 identity by superseding them, and reports the items only a decision or an arrival walk can settle | `src/curation/`, `src/commands/curation-migrate-command.ts`, `docs/adr/0018-…` |
