...
const usage =
  "Usage: academic-os pinned refresh --config <path> [--apply] [--json]";
...
function exitCodeForRefresh(report: PinnedRefreshReport): 0 | 1 | 2 {
  if (
...
function renderHuman(report: PinnedRefreshReport): string {
  const { counts } = report;
  return [
...
function closingLine(report: PinnedRefreshReport): string {
