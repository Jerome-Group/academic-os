...
| Pinned rewrite | The cohort-wide repair MF-AGENTS-004 names — what each module owes each pinned document, previewed with the first differing line, then written through proved mounted writes | `src/pinned/`, `src/commands/pinned-rewrite-command.ts` |
